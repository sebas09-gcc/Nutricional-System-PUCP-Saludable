...
{
    public partial class frmMostrarUbicación : Form
    {
        private Point pivotPoint;
        GMapOverlay markers = new GMapOverlay("markers");
        public frmMostrarUbicación((double, double, string) myLocation)
        {
            InitializeComponent();
            if (myLocation != (0, 0, null))
            {
                double latitude = myLocation.Item1;
                double longitude = myLocation.Item2;
                ...
                addMarkers();
                var resources = new ResourceManager(typeof(frmElegirUbicacion));
                var redpoint = new Bitmap((System.Drawing.Image)resources.GetObject("pointred"));
                GMapMarker marker = new GMarkerGoogle(new PointLatLng(myLocation.Item1, myLocation.Item2), redpoint);
                marker.ToolTipText = myLocation.Item3;
